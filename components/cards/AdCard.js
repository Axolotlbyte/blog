const AdCard = ({square}) => {
  return (
    <div className={"w-full cursor-pointer relative bg-slate-600 rounded-2xl" + (square ? " aspect-square" : " aspect-video")}>
      ...
      <p className=" pt-16 text-white w-full text-lg p-5">
      ...
      <button className="absolute hover:font-extrabold bottom-5 right-5 underline text-white font-semibold">
        Learn More
      </button>
    </div>
  );
